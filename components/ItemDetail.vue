...
<template>
        <div class="flex flex-col justify-between md:space-y-5 space-y-2">
            ...
            <div class="flex flex-row justify-between text-center text-xs md:text-base">
                <div class="md:space-x-2 space-x-1">
                    ...
                    }" v-for="size in allsize" :key="size.size" @click="selectSize(size.size)">
                        <div>{{ size.size }}</div>
                    </button>
                </div>
        ...
        <ModalProduct :targetSize="selectedSize" :targetId="Number(productId)" @toggleModal="openModalProduct" v-show="isOpenModal" />
</template>
<style scoped></style>